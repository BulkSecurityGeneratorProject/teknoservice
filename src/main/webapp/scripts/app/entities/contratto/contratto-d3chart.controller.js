...
.controller('ContrattoD3ChartController',['$scope', '$timeout', 'Contratto', function ($scope, $timeout, Contratto) {
	...
	Contratto.searchChart( $scope.$parent.contratto, function(result) {
    	$scope.data[0].values = result.map(function(d) { return {x: d[2], y: d[0] } });
    	$scope.data[1].values = result.map(function(d) { return {x: d[2], y: d[1] } });
    	//$scope.$apply();
    	/*$scope.data = $scope.data.map(function(series) {
